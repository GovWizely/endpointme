  ...
  describe 'ingest' do
    context 'basic upload with header conversion' do
      ...
      it 'creates entries in the new api index with the new class constant' do
        results = data_source.with_api_model do |klass|
          query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(country_name: 'Armenia').permit(:country_name))
    ...
    context 'specifying uniqueness constraints on column values' do
      ...
      it 'creates entries using an ID formed from a hash of use_for_id fields' do
        results = data_source.with_api_model do |klass|
          expect(klass.count).to eq(2)
          query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(country_name: 'United Kingdom').permit(:country_name))
          klass.search(query.generate_search_body_hash)
        end
    ...
    context 'specifying constant column value' do
      %w(json xml csv).each do |format|
        ...
        it 'creates entries with constant column value field' do
          results = data_source.with_api_model do |klass|
            expect(klass.count).to eq(2)
            query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(source: 'some constant value').permit(:source))
            klass.search(query.generate_search_body_hash)
          end
    ...
    context 'specifying copied fields' do
      %w(csv json xml).each do |format|
        ...
        it 'creates entries with field copied from source field and transformed' do
          results = data_source.with_api_model do |klass|
            expect(klass.count).to eq(2)
            query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(f3: 'VAL1FROMCOMMAS', f4: 'val1 to commas').permit(:f3, :f4))
            klass.search(query.generate_search_body_hash)
          end
    ...
    context 'grouping fields under parent fields' do
      ...
      it 'creates entries with grouped column value fields' do
        results = data_source.with_api_model do |klass|
          expect(klass.count).to eq(2)
          query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(title: 'bar').permit(:title))
          klass.search(query.generate_search_body_hash)
        end
    ...
    context 'specifying nested collections' do
      ...
      it 'creates entries with nested collections' do
        results = data_source.with_api_model do |klass|
          ...
          query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(params).permit(:mname, :lname, :q, :country_codes, :bday))
          klass.search(query.generate_search_body_hash)
        end
  ...
  describe 'search' do
    describe 'recall & relevancy' do
      ...
      describe 'recall' do
        it 'matches enum filter terms regardless of case' do
          results = data_source.with_api_model do |klass|
            query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(country_name: 'ANDORRA', iso2_codes: 'aD').permit(:country_name, :iso2_codes))
            klass.search(query.generate_search_body_hash)
          end
        ...
        it 'matches multiple filter terms separated by commas' do
          results = data_source.with_api_model do |klass|
            query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(iso2_codes: 'AD, ca').permit(:iso2_codes))
            klass.search(query.generate_search_body_hash)
          end
        ...
        it 'matches on fulltext regardless of accented characters' do
          results = data_source.with_api_model do |klass|
            query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(q: 'åccent').permit(:q))
            klass.search(query.generate_search_body_hash)
          end
          expect(results.size).to eq(1)
        end
    ...
    describe 'semantic query pre-processing' do
      ...
      it 'prioritizes semantic terms over fulltext match' do
        results = data_source.with_api_model do |klass|
          VCR.use_cassette('importers/data_sources/semantic_query_service/brazil.yml') do
            query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(q: 'brazil').permit(:q))
            klass.search(query.generate_search_body_hash)
          end
        end
    ...
    describe 'aggregations' do
      ...
      it 'returns expected aggregations' do
        results = data_source.with_api_model do |klass|
          query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(q: 'val').permit(:q))
          klass.search(query.generate_search_body_hash)
        end
  ...
  describe 'freshen' do
    ...
    context 'data has changed' do
      ...
      it 'inserts/updates data and deletes any existing records that are not in the updated data set' do
        ...
        data_source.with_api_model do |klass|
          expect(klass.count).to eq(2)
          query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(f1: 'one,three').permit(:f1))
          expect(klass.search(query.generate_search_body_hash).count).to eq(2)
          query = ApiModelQuery.new(data_source.metadata, ActionController::Parameters.new(f2: 'two,five').permit(:f2))
          expect(klass.search(query.generate_search_body_hash).count).to eq(2)
        end
      end
    end
